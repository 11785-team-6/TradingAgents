% Standalone results/analysis/discussion section.
...
The completed artifacts cover four evaluation windows: the full 2025 calendar year, a high-volatility shock window from 2025-02-24 to 2025-03-25, a bull breakout window from 2025-04-09 to 2025-05-08, and a bear drawdown window from 2025-10-27 to 2025-11-25.  The LLM runs use the \texttt{market}, \texttt{news}, and \texttt{fundamentals} analysts with \texttt{qwen3:8b} for fast reasoning and \texttt{qwen3:30b-a3b} for deeper reasoning.  All reported runs completed with zero execution errors.  The metrics-mode hybrid runs had zero failed leakage checks; the all-year signals-mode run had 7 failed checks out of 2555 audit checks, so signals-mode all-year conclusions should be interpreted with that caveat.

\begin{table}[h]
\centering
\caption{Full-year 2025 BTC results on the same signal calendar.  Returns and drawdowns are reported as percentages.}
\label{tab:all_year_results}
\begin{tabular}{lrrrr}
\toprule
...
On the full-year benchmark, the strongest standalone method is the LSTM baseline, which obtains a 92.6\% cumulative return and a Sharpe ratio of 2.24 while reducing maximum drawdown relative to buy-and-hold.  The pure LLM agent beats buy-and-hold in absolute and excess return, but only modestly: it returns 2.8\% with Sharpe 0.26.  The default signals-mode hybrid improves over the pure agent, reaching 14.7\% cumulative return and Sharpe 0.60, while the default metrics-mode hybrid underperforms.  The best all-year hybrid ablation is the metrics mid3 configuration, which exposes \texttt{arima\_garch}, \texttt{buy\_and\_hold}, and \texttt{xgb\_lstm\_ensemble} summaries to the agent; it reaches 30.8\% cumulative return and Sharpe 0.99.  Thus, hybridization helps only when the information interface and model subset are well matched.

\begin{figure}[h]
\centering
\includegraphics[width=0.98\linewidth]{figures/regime_cumulative_returns.pdf}
\caption{Cumulative return across the full-year and regime-specific windows for the principal method families.}
\label{fig:regime_returns}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.98\linewidth]{figures/regime_sharpe.pdf}
\caption{Annualized Sharpe ratio across the full-year and regime-specific windows.  Annualized values on 30-day windows should be read as directional risk-adjusted summaries rather than long-horizon performance estimates.}
\label{fig:regime_sharpe}
\end{figure}

\begin{table}[h]
\centering
\caption{Regime-specific cumulative returns and Sharpe ratios for representative strategies.}
